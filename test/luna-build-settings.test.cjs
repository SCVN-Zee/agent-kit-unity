...
// Runs via `node --test test/`. Lives outside kit/+scripts/ so it is neither
...
const mod = require("../kit/tiers/luna/skills/aku-luna-build-check/scripts/luna-build-settings.cjs");
...
const MESH_HALF = {
  key: "meshHalf",
  path: ["assets", "rules", "meshes", "default", "halfPrecision"],
  from: true,
  to: false,
};
...
test("applyFixes flags ambiguous (>1 match) and skips", () => {
  const raw =
    '{"meshes":{"default":{"halfPrecision":true}},"dup":{"meshes":{"default":{"halfPrecision":true}}}}';
  const { text, applied, skipped } = applyFixes(raw, [MESH_HALF]);
  assert.equal(applied.length, 0);

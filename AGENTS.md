...
- **File size**: each `.md` skill subfile and `.js`/`.cjs` source ≤200 LOC — **including `scripts/tests/`**, which `lint-loc` does not ignore. Split helpers into `scripts/lib/` as needed.
- **Capability refs**: reference Unity capabilities by **bare kebab ids in backticks** as illustrative labels — the agent binds them to the connected server's tools at runtime. The kit hard-codes no server name and emits no `mcp__<server>__` prefix.
...
- **One publisher:** `.github/workflows/release-stable.yml` and `release-beta.yml` are the only release publishers. They fail outside `SCVN-Zee/agent-kit-unity` but publish the exact tagged commit without constraining it to any branch ancestry, run `make check`, create a draft, upload exactly `install.sh`, `agent-kit-unity-v<version>.tgz`, and `SHA256SUMS`, verify the asset read-back, then publish. Stable becomes latest; beta remains a prerelease. Nothing publishes to npm.

...
const CONTRACTS = {
  'skills/aku-graft/SKILL.md': c('Use when', [/Graft code indexing/i, /project-local OMP MCP/i, /Git local exclude/i, /without changing \.gitignore/i, /Not for Unity Editor operations/i]),
  'rules/aku-core-rules.md': c('Always applies', [/MonoBehaviour lifecycle/i, /mobile performance/i, /every prompt/i], { alwaysApply: 'true' }),
  'rules/aku-asset-convention-rules.md': c('Use when', [/naming/i, /importing|folder organization|importer intent/i, /hierarchy/i, /serialized mutation/i], { globs: ['**/Assets/**', '.prefab', '.unity', '.asset', '.mat'] }),
  ...
  'tiers/luna/skills/aku-luna-build-check/SKILL.md': c('Use when', [/luna\.json/i, /six scene, AA, mesh, and animation gates/i, /not source or asset compatibility review/i]),
  'tiers/luna/skills/aku-luna-conventions/SKILL.md': c('Use when', [/Luna/i, /authoring/i, /editor-strip/i, /skill:\/\/aku-luna-code-review/i]),
  ...
  'tiers/luna/skills/aku-luna-code-review/SKILL.md': c('Use when', [/Luna/i, /Report-only/i, /Bridge\.NET/i, /skill:\/\/aku-luna-build-check/i]),
  'skills/aku-code-review/SKILL.md': c('Use when', [/report-only review/i, /Unity file, diff, commit, PR/i, /including Unity C#/i, /read-only verification/i, /loads skill:\/\/aku-code-conventions/i]),
  'skills/aku-asset-conventions/SKILL.md': c('Use when', [/naming/i, /importing|folder layout|importer intent/i, /hierarchy/i, /owns conventions, not mutation/i, /serialized operations/i])
};
...
function discover(ompRoot) {
  ...
  if (fs.existsSync(tiers)) for (const e of fs.readdirSync(tiers, { withFileTypes: true })) {
    ...
    addFiles(path.join(tiers, e.name, 'rules'), path.posix.join('tiers', e.name, 'rules'));
    const tierSkills = path.join(tiers, e.name, 'skills');
    if (fs.existsSync(tierSkills)) for (const s of fs.readdirSync(tierSkills, { withFileTypes: true })) {
      if (s.isDirectory()) found.push(path.posix.join('tiers', e.name, 'skills', s.name, 'SKILL.md'));
    }
  }
  return found.sort();
}
...
function validateTree(root = KIT_ROOT) {
  ...
  const counts = {
    ...
    tierRules: actual.filter((p) => p.startsWith('tiers/') && p.includes('/rules/')).length,
    tierSkills: actual.filter((p) => p.startsWith('tiers/') && p.includes('/skills/')).length
  };
...
function runCli(root = KIT_ROOT) {
  ...
  const summary = result.scanned + ' surfaces: ' + result.counts.skills + ' skills, ' + result.counts.baseRules + ' base rules, ' + result.counts.tierRules + ' tier rules, ' + result.counts.tierSkills + ' tier skills';

/**
 * Tests for omp-install-payload.js against the REAL packaged kit/ tree.
 *
...
const KIT_OMP = path.resolve(__dirname, "../../kit");
...
test("base payload includes AGENTS.md, rules/*, and skills/**", () => {
  ...
  assert.ok(
    base["skills/aku-code-conventions/ANIMATOR_DRIVING.md"],
    "a nested skill reference",
  );
});
...
test("convention split ships exact inventories without the legacy skill or URI", () => {
  const base = payload.baseFiles(KIT_OMP);
  assert.deepEqual(
    skillFiles(base, "aku-code-conventions"),
    CODE_CONVENTION_FILES,
  );
  assert.deepEqual(
    skillFiles(base, "aku-asset-conventions"),
    ASSET_CONVENTION_FILES,
  );
  assert.equal(
    skillFiles(base, "aku-conventions").length,
    0,
    "legacy skill must be absent",
  );
  ...
  for (const [dest, meta] of Object.entries(base)) {
    ...
    assert.equal(
      text.includes("skill://aku-conventions"),
      false,
      "legacy URI in " + dest,
    );
  }
...
test("generic router is absent from every shipped payload file", () => {
  ...
  assert.equal(
    skillFiles(base, retired).length,
    0,
    "retired skill directory must be absent",
  );
  for (const [dest, meta] of Object.entries(base)) {
    if (!/\.(?:c?js|json|md)$/.test(dest)) continue;
    const text = fs.readFileSync(meta.srcAbs, "utf8");
...
test("base payload excludes README.md and tiers/", () => {
  const base = payload.baseFiles(KIT_OMP);
  assert.ok(!base["README.md"], "kit/README.md must not be installed");
  for (const k of Object.keys(base)) {
    assert.ok(
      !k.startsWith("tiers/"),
      `tiers must not be in the base set: ${k}`,
    );
  }
});
...
test("tier overlay maps to rules/<name> tagged with the tier", () => {
  ...
  assert.ok(
    tf["rules/aku-sc-rules.md"],
    "supercent rule at rules/aku-sc-rules.md",
  );

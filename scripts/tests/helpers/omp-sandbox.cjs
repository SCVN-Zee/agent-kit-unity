 ...
 * proves the packaged kit/ is resolved from the module root, not from cwd).
 */
...
// Run from os.tmpdir() (a cwd with no kit/) so kitOmpDir must resolve from the
// module package root for the install to find any source at all.
function ship(target, args = []) {
 const r = spawnSync("node", [SHIP, target, ...args], {
  encoding: "utf8",
  cwd: os.tmpdir(),
 });
...
function omp(target, rel) {
 return path.join(target, ".omp", rel);
}
function readLock(target) {
 return JSON.parse(fs.readFileSync(omp(target, "aku-lock.json"), "utf8"));
}
function sha(buf) {

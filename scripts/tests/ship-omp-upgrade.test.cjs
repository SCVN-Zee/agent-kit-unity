...
const {
  ...
  sha,
  cleanup,
} = require("./helpers/omp-sandbox.cjs");
...
test("--tier luna installs both Luna skills flattened, tier-tagged, bytes intact", () => {
  ...
  try {
    ...
    for (const rel of [
      "skills/aku-luna-build-check/SKILL.md",
      "skills/aku-luna-build-check/scripts/luna-build-settings.cjs",
      "skills/aku-luna-code-review/SKILL.md",
      "rules/aku-luna-rules.md",
    ]) {
      assert.ok(fs.existsSync(omp(t, rel)), `missing ${rel}`);
    }
    const script =
      "skills/aku-luna-build-check/scripts/luna-build-settings.cjs";
    const src = path.join(__dirname, "../../kit/tiers/luna", script);
    assert.deepEqual(
      fs.readFileSync(omp(t, script)),
      fs.readFileSync(src),
      "byte-identical .cjs",
    );
    ...
    assert.equal(
      lock.files["skills/aku-luna-code-review/SKILL.md"].tier,
      "luna",
    );
  } finally {
    cleanup(t);
...
test("former base Luna skills prune on a non-Luna update only when bytes match", () => {
  ...
  try {
    ...
    const src = path.join(__dirname, "../../kit/tiers/luna", editedRel);
    lock.files[editedRel] = {
    ...
    lock.files[retiredRel] = {
      hash: sha(retiredBytes),
      installedAt: now,
      updatedAt: now,
    };
    fs.writeFileSync(
    ...
    assert.equal(r.code, 0, r.out);
    assert.ok(
      !fs.existsSync(omp(t, "skills/aku-luna-build-check/SKILL.md")),
      "clean luna skill pruned (I==L)",
    );
    assert.ok(
      !fs.existsSync(
        omp(t, "skills/aku-luna-build-check/scripts/luna-build-settings.cjs"),
      ),
      "nested tier script pruned (I==L)",
    );
    assert.ok(
      !fs.existsSync(omp(t, retiredRel)),
      "retired-name skill pruned (I==L)",
    );
...
test("upgrade: retired aku-engine-rules.md prunes when unmodified, renamed rule installs", () => {
  ...
  try {
    ...
    const lock = readLock(t);
    const now = new Date().toISOString();
    lock.files["rules/aku-engine-rules.md"] = {
      hash: sha(legacy),
      installedAt: now,
      updatedAt: now,
    };
    ...
    assert.equal(r.code, 0, r.out);
    assert.ok(
      !fs.existsSync(omp(t, "rules/aku-engine-rules.md")),
      "retired rule pruned (I==L)",
    );
    assert.ok(
      fs.existsSync(omp(t, "rules/aku-core-rules.md")),
      "renamed rule installed",
    );
    const after = readLock(t);
    assert.ok(
      !after.files["rules/aku-engine-rules.md"],
      "lock no longer records the retired rule",
    );
    assert.ok(
      after.files["rules/aku-core-rules.md"],
      "lock records the renamed rule",
    );
...
test("upgrade keeps a user-edited retired rule as a conflict", () => {
  ...
  try {
    ...
    const edited = Buffer.concat([
      Buffer.from("# Unity Rules (sticky)\n"),
    ...
    const now = new Date().toISOString();
    lock.files["rules/aku-engine-rules.md"] = {
      hash: sha(Buffer.from("# Unity Rules (sticky)\n")),
      installedAt: now,
      updatedAt: now,
    };
    delete lock.files["rules/aku-core-rules.md"];
    ...
    assert.equal(r.code, 0, r.out);
    assert.deepEqual(
      fs.readFileSync(omp(t, "rules/aku-engine-rules.md")),
      edited,
      "edited retired file kept",
    );
    const after = readLock(t);
    assert.equal(
      after.files["rules/aku-engine-rules.md"].orphaned,
      true,
      "kept as orphaned conflict",
    );
    assert.ok(
      after.files["rules/aku-core-rules.md"],
      "renamed rule still installs",
    );
  } finally {
    cleanup(t);
  }

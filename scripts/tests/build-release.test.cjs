...
test("release metadata accepts only exact stable and positive beta tags", () => {
  ...
  for (const version of [
    ...
    assert.throws(
      () => validateRelease(metadata(version)),
      /unsupported version/,
    );
});
...
test("release metadata rejects channel, tag, lock, and repository disagreement", () => {
  assert.throws(
    () => validateRelease(metadata("0.1.0", "v0.1.0", "beta")),
    /not beta/,
  );
  assert.throws(
    () => validateRelease(metadata("0.1.0", "v0.1.1")),
    /must equal/,
  );
...
test("archive closure fails when any runtime module or OMP surface is absent", () => {
  const complete = [
    ...REQUIRED,
    "package/kit/rules/rule.md",
    "package/kit/skills/skill/SKILL.md",
  ];
  ...
  assert.throws(
    () =>
      assertArchiveEntries(
        complete.filter((entry) => !entry.includes("/rules/")),
      ),
    /rules/,
  );
  assert.throws(
    () =>
      assertArchiveEntries(
        complete.filter((entry) => !entry.includes("/skills/")),
      ),
    /skills/,
  );
});
...
test("installer rendering is deterministic and rejects unsafe or unresolved values", () => {
  ...
  assert.throws(
    () => renderInstaller("@@TAG@@", { TAG: "v0.1.0'; echo unsafe" }),
    /unsafe/,
  );
...
test("CLI parser requires explicit known channel options", () => {
  assert.deepEqual(
    parseCli(["v0.1.0", "--channel", "stable", "--validate-only"]),
    {
      tag: "v0.1.0",
...
test("make bump strips a leading v and no longer requires a changelog section", () => {
  ...
  try {
    ...
    fs.copyFileSync(
      path.join(root, "scripts", "build-release.cjs"),
      path.join(dir, "scripts", "build-release.cjs"),
    );
    for (const name of ["global-install-fs.js", "release-metadata.js"]) {
      fs.copyFileSync(
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: "aku-bump-fixture", version: "0.1.0-beta.7" }),
    );
    fs.writeFileSync(
      path.join(dir, "README.md"),
      "install: https://github.com/SCVN-Zee/agent-kit-unity/releases/download/v0.1.0-beta.7/install.sh\n",
    );
    const r = spawnSync("make", ["bump", "VERSION=v0.1.0-beta.8"], {
      cwd: dir,
      encoding: "utf8",
    });
    assert.match(
      r.stdout,
      /v0\.1\.0-beta\.8/,
      `bump stopped before npm version — stderr: ${r.stderr}`,
    );
    assert.doesNotMatch(r.stderr, /CHANGELOG\.md/);
    assert.notEqual(
      r.status,
      0,
      `expected fixture failure after the removed gate, stderr: ${r.stderr}`,
    );
  } finally {

/**
 * omp-install-payload.js — enumerate the packaged `kit/` content into the set of
 ...
 * The kit's own kit/README.md and kit/tiers/ are deliberately excluded from the
...
/** Tier overlay: kit/tiers/<tier>/rules/* → dest rules/<name>, and
 *  kit/tiers/<tier>/skills/** → dest skills/<subpath>; both tagged {tier}. */
function tierFiles(kitOmpDir, tier) {
 ...
 for (const abs of walkFiles(skillsDir)) {
  const dest =
   "skills/" + path.relative(skillsDir, abs).split(path.sep).join("/");
  out[dest] = { srcAbs: abs, hash: hashFile(abs), tier };
 }
 return out;
}

/** Tier names shipped under kit/tiers/ (sorted directory listing). */
function availableTiers(kitOmpDir) {
 try {
  return fs
   .readdirSync(path.join(kitOmpDir, "tiers"), { withFileTypes: true })
...
function computePayload(kitOmpDir, tiers = []) {
 ...
 for (const tier of tiers) {
  ...
  for (const [dest, meta] of Object.entries(tf)) {
   if (payload[dest]) {
    throw new Error(
     `payload dest collision at ${dest}: tier ${tier} vs ${payload[dest].tier || "base"}`,
    );
   }
   payload[dest] = meta;
  }

 ...
 *
 * Walks kit/ and scripts/. Skips node_modules and tests fixtures.
 */
...
const ROOTS = [path.join(KIT_ROOT, "kit"), path.join(KIT_ROOT, "scripts")];
...
(function main() {
  ...
  console.error(
    `lint-loc: ${offenders.length} offender(s) over ${HARD_CAP} LOC:`,
  );
  for (const o of offenders.sort((a, b) => b.lines - a.lines)) {
    console.error(`  ${o.lines.toString().padStart(4, " ")}  ${o.p}`);
  }
  console.error(
    "Modularize or add to CARVE_OUTS in scripts/lint-loc.cjs with justification.",
  );
  process.exit(2);
})();

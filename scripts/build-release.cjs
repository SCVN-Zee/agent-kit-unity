...
const { STABLE, BETA, validateRelease } = require("./lib/release-metadata");
const REQUIRED = [
  "package/package.json",
  "package/kit/AGENTS.md",
  "package/scripts/ship-kit.cjs",
  ...
  ...[
    ...
    "install-safety",
    "startup-section",
  ].map((name) => `package/scripts/lib/host-${name}.js`),
];
...
function renderInstaller(template, values) {
  ...
  if (/@@[A-Z_]+@@/.test(rendered))
    throw new Error("unresolved installer template placeholder");
  return rendered;
}
...
function assertArchiveEntries(entries) {
  for (const rel of REQUIRED) {
    if (!entries.includes(rel))
      throw new Error(`release archive missing ${rel}`);
  }
  for (const prefix of ["package/kit/rules/", "package/kit/skills/"]) {
...
function assertArchive(archive, root) {
  return assertArchiveEntries(
    run("tar", ["-tzf", archive], root).trim().split("\n"),
  );
}
...
function buildRelease({
  ...
  try {
    ...
    copyEntry({
      src: path.join(staging, packed[0].filename),
      dest: archive,
      kind: "file",
    });
    ...
    const template = fs
      .readFileSync(
        path.join(root, "scripts/release/install.sh.template"),
        "utf8",
      )
      .replace(
        "@@CHANNEL_PATTERNS@@",
        () => `const patterns = { stable: ${STABLE}, beta: ${BETA} };`,
      );

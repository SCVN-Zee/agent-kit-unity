...
## 2. Identity safety and house policy

**Unity serializes an enum as its underlying integer.** The member *name* is never written to the asset. So
changing implicit numbering can silently rewrite the meaning of data already on disk:
...
Explicit numbering (`Stun = 0, Slow = 1`) preserves surviving values across insertion/removal. Reserve retired
values rather than reusing them, and migrate assets containing removed members. Flags likewise keep explicit bits;
removal does not require renumbering. House policy still uses data-owned identities for content-defined sets.

A picker decouples display order from stored identity, but renamed strings or reused integer IDs still need
migration. Prefer stable IDs/object references and validate membership against the same authority.
...
| stable | identity | multi | `[Flags]` enum with stable explicit bits (32 for int backing) | `[EnumToggleButtons]` |
...
| — | Existing supported type | — | `LayerMask`, `AnimationClip`; `AssetReference` only with Addressables installed | none |
...
1. **Volatile set as an enum** — violates data-ownership policy. Report remap corruption only with the numeric-change evidence in §2.
...
3. **`[Flags]` on a volatile set** — an int-backed mask has 32 bits; renumbering changes stored meanings. A
   volatile multi-select set is a checkbox list (§3 row 4), never a flags enum.
4. **Content taxonomy flattened into an enum** — hierarchy alone does not prove churn. Establish whether
   members are authored as data or require new code, then apply the switch test.
...
First establish whether stored integers are already wrong or the migration is preventive. Deleting the enum does not
migrate the assets that reference it. Safe order: add the new representation alongside the enum, migrate the

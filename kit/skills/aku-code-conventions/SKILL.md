...
6. **Lifecycle pairing:** release owned resources/subscriptions at their lifetime boundary; `Init`↔`Release`, pool return distinct from `OnDestroy`.
7. **Comments explain why, not what.**
8. **Inspector-wire scene-time references.** No runtime `GetComponent` or `Find*`, including `Awake`/`Start`; only post-`Instantiate()` lookup is allowed. Editor-only setup may use bounded lookup and must preserve Undo and prefab overrides; dirty state is not saving.
...
12. **Every serialized reference is required unless explicitly optional.** With Odin, place `[Required]` in its own bracket; document optional null behavior with `[PropertyTooltip]`. Use bounds for value types; validate collection count and elements separately. Without Odin, assert on the actual initialization path and name the field.

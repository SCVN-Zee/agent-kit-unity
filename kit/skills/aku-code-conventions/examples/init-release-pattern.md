...
```csharp
using Sirenix.OdinInspector; // Odin installed; otherwise use initialization assertions.
using UnityEngine;
...
public class Player : MonoBehaviour
{
    ...
    // Serialized Fields
    //----------------------------------------------------------------------
    // Illustrative ID source: replace with the project's authority-backed picker or config reference.
...
For pooling, the owner must call `Release()` on pool return and `Init()` on acquire; OnDestroy alone is not enough.
The idempotency flag covers duplicate successful calls, not partial initialization failures. Production acquisition
must roll back already-created resources if a later step throws, or use an explicit partial-state release contract.

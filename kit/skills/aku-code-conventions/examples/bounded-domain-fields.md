...
recipe 7 covers player-build provider mechanics. These short provider fragments illustrate authority discovery;
cache repeated expensive results and invalidate them as described in `skill://aku-odin/PICKERS_AND_VALIDATION.md`.
...
the registry, rather than the hierarchy spelling, establishes data ownership.
...
private static readonly ValueDropdownList<string> GAS_TAG_CHOICES = new ValueDropdownList<string>
{
...
[ValueDropdown(nameof(GetTags))]                 // nameof is bound even if attribute metadata is omitted

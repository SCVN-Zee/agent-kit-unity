...
Discovery fragments below are not a repaint-cache implementation. Use the invalidation contract in
`skill://aku-odin/PICKERS_AND_VALIDATION.md` for repeatedly evaluated or expensive authorities.
...
{
    return !string.IsNullOrEmpty(statePath) &&
        statePath.StartsWith(_previewLayer + ".", System.StringComparison.Ordinal);
}
```
...
The state provider covers top-level states only; recurse for nested state machines. The prefix predicate checks
layer format, not existence: validate actual membership against the same authority before treating a selection as valid.

...
## 4. Existing hierarchy safety

Rename existing objects only within the requested scope. Animation clip bindings, `Transform.Find` paths and editor tooling may depend on hierarchy names or parent paths; check these consumers before and after changes. A prefab asset rename and its root GameObject rename are separate decisions.

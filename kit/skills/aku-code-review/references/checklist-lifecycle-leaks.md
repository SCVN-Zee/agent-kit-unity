...
- A pool return does not invoke OnDestroy. Check per-use reset/release independently of final destruction.
- Partial Init failures need rollback; a boolean set only at the end cannot release already-acquired resources by itself.

## Scene / singleton hazards

- Static field holding a scene object reference across a scene load → stale ref + leak. With domain reload disabled, reset static caches/events explicitly between play sessions.
...
- Managed temporary editor allocations outside hot paths. Editor-created textures, property trees and event subscriptions still need ownership/cleanup.

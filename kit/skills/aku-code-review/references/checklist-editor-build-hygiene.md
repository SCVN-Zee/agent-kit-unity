...
- A `Setup Refs` / auto-wire method (populates `[SerializeField]` slots via `GetComponent`/`Find`, or calls `UnityEditor.*`) NOT wrapped in `#if UNITY_EDITOR` → build break / ships edit-time logic. Check serialized-property apply or direct-write Undo/dirty handling and prefab-instance modification recording; dirty marking is not saving. Correct trigger: `[Button]`/`Reset` (`[ContextMenu]` without Odin), never `OnValidate` (cross-ref `skill://aku-code-conventions/REFERENCE_WIRING.md` §2–§3).
...
## Editor UX and persistence

- Verify Undo/redo, prefab instance/variant overrides and reload persistence for actual mutation surfaces; read-only review reports missing evidence rather than exercising writes.
- Attribute filtering is not data validation. Check empty/stale dropdown choices, null collection elements and mixed multi-selection.
- Check keyboard use, narrow widths, labeled destructive actions, empty states and clear asset ownership for inline editing.
- Repaint providers should avoid repeated scans; cached icons/trees and event subscriptions still need cleanup/invalidation.
- Optional Odin modules and Visual Designer layouts need installed-version checks; rendering does not imply serialization.

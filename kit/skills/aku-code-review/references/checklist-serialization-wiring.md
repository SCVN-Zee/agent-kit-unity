...
- ScriptableObject exposing mutable state as `public` field instead of `[field: SerializeField] public T Prop { get; private set; }` (Inspector-editable, external reassignment restricted; referenced data can still be mutable).
...
- Several refs resolved via `GetComponent`/`Find` in `Awake`/`Start` that should be `[SerializeField]` slots populated by an editor-only `Setup Refs` method (`[Button]`/`Reset`; `[ContextMenu]` without Odin) — the sanctioned edit-time auto-wire (cross-ref `skill://aku-code-conventions/REFERENCE_WIRING.md` §2–§3).
- A serialized reference the code dereferences without a null check, where the slot can be left empty in the Inspector. Route by Odin presence: **Odin installed** → `[Required]` (edit-time error box, the mandated answer); **Odin absent** → `Debug.Assert` on the actual initialization path naming the field, the sanctioned degraded tier. Keep runtime guards when destruction, code assignment or external data can still invalidate the reference; an Inspector error is not a runtime guarantee.
...
- Dictionary drawing mistaken for persistence — verify the installed Unity version and serializer's supported shape. Older Unity versions need a serializable backing representation; newer support is version-specific.
- Managed interface/abstract data needing `[SerializeReference]` — eligible instances must be serializable managed classes, not UnityEngine.Object implementations. Component references need concrete/base object slots plus interface validation.
...
- **DEFECT — volatile value set typed as an `enum`.** A set whose members arrive as *content* (GAS tags, item ids, ability keys — no new code path per member) declared as an `enum`. Implicit numbering can repoint saved values when members move. Stable explicit values prevent renumbering; removed values still require migration or reserved tombstones. Treat content ownership as house policy, and establish actual numeric changes before reporting data corruption. Fix: picker or `ScriptableObject` ref per the decision table.
  - *Illustrative failure scenario — confirm that the reviewed enum actually uses implicit numbering:* `enum EDebuff { Stun, Slow }` with 50 assets authoring `Slow` (= 1); a later `enum EDebuff { Stun, Frozen, Slow }` makes all 50 read `Frozen` — no compile error, no warning, no diff in the `.asset`. Canonical statement of the mechanism stays in `skill://aku-code-conventions/BOUNDED_DOMAIN_FIELDS.md` §2.
- **DEFECT — `[Flags]` on a volatile set.** House policy prefers data-owned multi-selection: renumbering/reusing bits changes stored combinations. Removal does not force renumbering: preserve retired bits. An int-backed mask has 32 bits; use content-owned identities for an open data set. A volatile multi-select set is a checkbox list (`IsUniqueList`), never a flags enum.
...
- Renamed serialized fields/auto-property backing fields without a tested migration can lose data or prefab overrides; inspect FormerlySerializedAs usage and save/reopen evidence.
- ShowInInspector, TableMatrix or a dictionary drawer alone does not establish persistence. Verify backing storage and reload behavior.
...
- **`[Required]` absent from a collection** is not itself a defect. Validate required count, null elements and uniqueness separately. Attribute propagation to elements is attribute/version-specific; do not flag Required as a universal no-op. Use RequiredListLength where supported or an explicit collection predicate.

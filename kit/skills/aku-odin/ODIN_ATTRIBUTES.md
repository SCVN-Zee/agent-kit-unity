...
Confirm actual API/assembly availability before emitting symbols; a policy override cannot make a missing DLL compile.
...
| `[Header("Stats")]` | `[BoxGroup("Stats")]` / `[FoldoutGroup("Stats")]` | grouping upgrade — groups only attributed members at the same path; not an implicit wrapper around following fields |
...
| Collection presentation | `[ListDrawerSettings]`, `[TableList]`; dictionary/matrix drawing requires separately proven backing storage |
...
and carry perf and portability consequences the display attrs do not. A separate architecture/migration decision — raise it with the project owner, do not auto-adopt.
...
| `"GetTags"` (string literal) | no compile-time member binding | compiles; an editor resolver error occurs only if the provider is absent in the editor |

**Rule:** when the attribute expression remains in source, keep its `nameof` provider/predicate compiled and make only the editor-dependent body conditional. Conditional attribute metadata omission does not skip C# name binding. A target rule may guard the attribute together with its provider; never guard away the serialized field.
...
`[PropertyTooltip]` opt-out, the `[RequiredIn]` prefab tier, separate collection count/element checks, and the no-Odin assert tier live
...
Two rules are load-bearing for it: §1 (the presence gate) and §6 (keep `nameof` targets available wherever the
attribute expression is compiled — collection predicates follow the same contract).

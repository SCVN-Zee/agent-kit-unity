...
| Same-schema rows compared by columns | `[TableList]` | order or heterogeneous detail matters more than comparison |
| Spatial grid editing | `[TableMatrix]` with proven backing storage | the data is an ordinary row list or cannot survive reload |
...
Avoid repeated reflection/asset scans, avoidable allocations and texture creation in frequently evaluated getters,
conditions, providers and repaint callbacks. Cache expensive choices with explicit invalidation; small bounded
allocations are not automatically a defect. See [`PICKERS_AND_VALIDATION.md`](PICKERS_AND_VALIDATION.md).
...
For concrete group paths/actions, use [`GROUPING_AND_ACTIONS.md`](GROUPING_AND_ACTIONS.md); for collection/storage
choices, use [`COLLECTIONS_AND_REFERENCES.md`](COLLECTIONS_AND_REFERENCES.md). Verify multi-selection, keyboard use,
Undo/redo and persistence, not just appearance. Required consumer evidence lives in [`VERSION_AND_MODULES.md`](VERSION_AND_MODULES.md).

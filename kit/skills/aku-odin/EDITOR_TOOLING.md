...
| `SdfIcons` | `Sirenix.OdinInspector.Editor` (**editor-only**) | Editor assembly/guard |
| `SirenixEditorGUI` | `Sirenix.Utilities.Editor` (**editor-only**) | Editor assembly/guard |
...
a bare trash glyph is a support ticket. Icons should improve recognition, not increase visual density.

## 5. Inspector composition and tabs

Tabs belong to Inspector information architecture, not window chrome. Use [`INSPECTOR_UX.md`](INSPECTOR_UX.md) for the layout ladder, stable-domain test, state semantics, narrow-width rules, and [`examples/grouped-component.md`](examples/grouped-component.md).
...
- prefer serialized-property edits; for direct writes record Undo first, mark assets dirty as appropriate and record prefab-instance modifications; saving is separate,
...
private static void Open()
{
    ...
    window.titleContent = new GUIContent("Level Builder", window.Icon);
}
```

The window owns this generated texture: destroy it on disable/reload and recreate on enable. Do not destroy a shared
asset icon. Full lifecycle template: [`examples/editor-tool-window.md`](examples/editor-tool-window.md).

Prefer DrawEditors or OnInspectorGUI for custom GUI. If overriding OnGUI is unavoidable, preserve base drawing;
otherwise Odin's automatic body is lost. Check installed lifecycle signatures and call base overrides.

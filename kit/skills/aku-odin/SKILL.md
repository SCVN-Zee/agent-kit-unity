...
| [`GROUPING_AND_ACTIONS.md`](GROUPING_AND_ACTIONS.md) | Group paths, responsive tabs/actions and mode semantics. |
| [`PICKERS_AND_VALIDATION.md`](PICKERS_AND_VALIDATION.md) | Choice authority, cache invalidation, selectors and actionable validation. |
| [`COLLECTIONS_AND_REFERENCES.md`](COLLECTIONS_AND_REFERENCES.md) | Lists/tables, inline editing, previews and storage boundaries. |
| [`VERSION_AND_MODULES.md`](VERSION_AND_MODULES.md) | Version checks, Visual Designer, UI Toolkit, Addressables, Validator and consumer evidence. |
| [`examples/grouped-component.md`](examples/grouped-component.md) | Generic task-oriented component; no tier-specific guards. |
| [`examples/collection-config.md`](examples/collection-config.md) | Serializable table rows with explicit validation. |
| [`examples/editor-tool-window.md`](examples/editor-tool-window.md) | Focused window and owned icon lifecycle. |
| [`examples/menu-editor-window.md`](examples/menu-editor-window.md) | Asset navigation with explicit refresh and valid content roots. |

Load the relevant reference for the task, not every file. Examples are illustrative until compiled and exercised in
the consuming Unity/Odin versions. New APIs and optional modules require [`VERSION_AND_MODULES.md`](VERSION_AND_MODULES.md).
...
Confirm presence per `AGENTS.md`: `Assets/Plugins/Sirenix/` exists, or `Packages/manifest.json` / an asmdef references Sirenix. **Override:** `.omp/aku-project.json {"odin": true|false}` — committed, beats auto policy detection. It does not prove DLL/API availability; confirm assembly references and version before emitting symbols.

...
Do not create a property tree per repaint. Cache one per stable target set, dispose before replacing it and on teardown.
`Draw()` defaults to Undo handling; do not copy tutorial `Draw(false)` into an asset editor without understanding why.
For selectors, use SelectionChanged for preview, SelectionConfirmed for committing and SelectionCancelled to preserve
original data. Live-preview mutation needs explicit rollback and Undo ownership.
...
- Prefer serialized-property editing. Direct writes need Undo before assignment and prefab-instance modification recording afterward when applicable; mark persistent assets dirty as appropriate. Saving is separate and authorized.
- Use `SerializedObject`/Odin property APIs when multi-object or prefab-override behavior matters.
- Cache SdfIcons textures, menu pages, selectors and property trees at their ownership lifetime; destroy/dispose owned resources on teardown/reload.
...
See [`VERSION_AND_MODULES.md`](VERSION_AND_MODULES.md) for optional products, UI Toolkit coexistence and consumer
verification, and [`PICKERS_AND_VALIDATION.md`](PICKERS_AND_VALIDATION.md) for selection/validation boundaries.

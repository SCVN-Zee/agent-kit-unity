...
- **Unity version + render pipeline** — read `ProjectSettings/ProjectVersion.txt` for the exact version. Check Graphics/Quality render-pipeline assignments and `Packages/manifest.json` for Built-in, URP, HDRP or custom SRP; an installed package alone does not prove the active pipeline. State the detected target and pipeline before suggesting render-specific code.
